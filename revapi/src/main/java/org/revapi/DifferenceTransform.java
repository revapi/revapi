 ...
 *
 * @param <E> the type of the element expected in the {@code transform} method. Note that you need to be careful about
...
public interface DifferenceTransform<E extends Element<?>> extends AutoCloseable, Configurable {
     ...
     * all elements and its {@link TraversalTracker#endTraversal()} has been called.
     * Only after this method has been called on all element pairs from the traversal,
     * the {@link #endTraversal(TraversalTracker)} is called to potentially clean up the resources before the traversal
     ...
     * of analysis may start using a different API analyzer. This is advertised to the transforms by calling
     * {@link #startTraversal(ApiAnalyzer, ArchiveAnalyzer, ArchiveAnalyzer)} again.
     *
     * @param tracker the traversal tracker returned from
     * {@link #startTraversal(ApiAnalyzer, ArchiveAnalyzer, ArchiveAnalyzer)} or null if that method returned an
     *                empty optional.
     */
