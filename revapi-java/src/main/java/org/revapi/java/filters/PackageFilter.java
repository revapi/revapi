 ...
 */
@Deprecated
public class PackageFilter extends IncludeExcludeFilter {

    public PackageFilter(Pattern[] includes, Pattern[] excludes) {
        super(includes, excludes);
    }

    private PackageElement findPackage(JavaElement el) {
        TypeElement t = findType(el);
        return t == null ? null :  el.getTypeEnvironment().getElementUtils().getPackageOf(t);
    }
    ...
    protected String getMatchableRepresentation(JavaElement el) {
        PackageElement pkg = findPackage(el);
