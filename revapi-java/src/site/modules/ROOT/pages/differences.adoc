...
This is only checked for new methods. The problem here is that if there are 2 overloads of a method that differ only in the type of
the vararg parameter, then trying to call one of such methods with empty varargs results in an ambiguity and compilation error.
