...
While the JLS ensures this is a binary compatible change, it can be source incompatible in a situation where
an inheriting class overrides the method with stricter visibility. E.g. if you have a `protected` method that you want
to make `public`, it will break all the classes that override your method with `protected` visibility.
...
[[java.method.varargOverloadsOnlyDifferInVarargParameter]]
=== Vararg method only differs in the vararg type from some of its overloads - `java.method.varargOverloadsOnlyDifferInVarargParameter`
[cols="35s,<65d"]
|=============
| Binary severity   | non breaking
| Source severity   | non breaking
...
This is only checked on the methods in the new version of the API. The problem here is that if there are 2 overloads of
a method that differ only in the type of the vararg parameter, then trying to call one of such methods with empty
varargs results in an ambiguity and compilation error.

By default, Revapi reports this only on the new version of the method, because it is assumed that the old version is
already out in the wild. This still may not be desirable behavior though if you need to keep the backwards compatibility
with the old version and therefore want to keep the "problem" in the API. In such cases, you can configure the check to
only report the problem on methods newly added to the new API by configuring the API check:
