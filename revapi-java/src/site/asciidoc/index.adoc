...
==== Default Serialization of Class Changed
[cols="35s,<65d"]
|=============
...
No API breakage, provided for completeness sake. Note that this is reported only for publicly accessible fields.
...
Note that this is reported only for publicly accessible fields.
...
The method is no longer default. This means that any implementations of the interface with the method
...
The situation with `Implementation.method()` and `JustAClass.definitelyAPIMethod()` is actually quite similar. In
...
The Java extension can be augmented through its small link:../revapi-java-spi/index.html[SPI]. Check it out for some
