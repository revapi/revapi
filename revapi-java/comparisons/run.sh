...
#-------------------------- clirr --------------------------------------
start=$(($(date +%s%N)/1000000))
$JAVA_HOME/bin/java -jar $CLIRR_DIR/clirr-core-0.6-uber.jar -o v1.jar -n v2.jar > clirr.report
end=$(($(date +%s%N)/1000000))
...
cd "$INST_DIR"
tar xjf "$DIR"/../../revapi-standalone/target/revapi-${REVAPI_VERSION}-standalone.tar.bz2
cd revapi-${REVAPI_VERSION}
# dry run so that revapi downloads its extensions and so we have a fair comparison of times
./revapi.sh -e org.revapi:revapi-java:${REVAPI_VERSION},org.revapi:revapi-reporting-text:${REVAPI_VERSION} \
